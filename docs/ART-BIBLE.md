...
- Use transparent-background character sprites for people.
...
- Production generation path: ChatGPT image generation using the approved GPT Image 2 workflow under Armaan's subscription. Paid API generation is not approved for this phase.
...
Approved concept-board prompt references for the 12-character Season 1 cast. Each concept board must produce exactly 12 options and must stay inside `tower-flat-plus-depth-v1`.
...
Pending approval. Generate exactly 12 distinct Otis concepts before production integration. Each concept should vary meaningfully in age impression, posture, uniform, silhouette, warmth, and sprite read while staying inside `tower-flat-plus-depth-v1` and fitting the existing Lobby backgrounds.
...
Required first-board variants:

1. Classic burgundy concierge, older and composed.
2. Younger modern front-desk host, tailored but relaxed.
3. Tall elegant maître d' silhouette, formal and quiet.
4. Warm retired-professor concierge, bookish and reassuring.
5. Cinematic realistic reception lead, subtle uniform.
6. Painterly editorial character, strong brass lighting.
7. Noir desk attendant, sharper silhouette, softer face.
8. Luxury hotel veteran, waistcoat and guest book.
9. Minimal graphic editorial direction, clean shape language.
10. Human-centered app sprite direction, approachable at mobile size.
11. Premium game-dialogue sprite direction, expressive but not fantasy.
12. Architectural-lobby integrated pose, hand resting near brass bell.

### art-bible:otis-pose-pack-v1

Blocked until an Otis concept winner reference is approved. Required outfit variants are `regular`, `summer-light`, and `winter-layered`; they must be edits of the approved Otis concierge outfit, not new outfits. Required poses are `idle`, `greeting`, `listening`, `thinking`, `talking`, `alert`, and `working` for each outfit variant. The batch factory must create a 4K transparent master, dark/light QA preview, default WebP, `@2x`, and `@3x` derivative for every sprite before the final upload-ready board can be approved for `public/art/lobby/otis/<outfitVariant>/`.
