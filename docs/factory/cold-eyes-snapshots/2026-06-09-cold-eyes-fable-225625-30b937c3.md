...
## Annex — cross-model teardown (GPT-5.x, commissioned for this review)

*Run against the same corpus by a non-Claude family, completed after the findings above
were drafted and written. A separate, independently produced Codex verdict
(`2026-06-09-cold-eyes-codex.md`, 22:53) also exists in this folder; its bottom line
converges ("the strongest thing here is a small process core," not the theme, the Base,
or the fleet story).*

**Convergent across all three model families (highest-confidence findings):**
- Memoirs-before-battle. His framing: "~5,500 lines of self-describing operating
  mythology around a checklist that has never executed once" — verified on disk: no
  campaigns/, no ORDERS.md, no WAR-LOG.md, no AAR, empty VERDICTS.md, nothing in the
  compounding layer the system generated itself.
- The kernel installed backwards (finding 4) — with a sharper point: the hook is a
  static `echo` of a constant string, which AT-MAX §1 describes as deterministic routing
  machinery — "dresses a constant-string injection as an inference engine." And by the
  doctrine's own cold-start law (early campaigns cheap, reduced ceremony), the kernel is
  the *maximal* violation: maximum standing ceremony, every message, at campaign count
  zero.
- The settings.json `bypassPermissions` contradiction (finding 4) — independently
  verified. "A Red-class action is currently gated only by whether a model loaded and
  obeyed a markdown file."
- Provenance-washing (finding 6) — plus: the genuinely load-bearing operational claims
  (the 06-15 billing split, Routines quotas, window pricing) carry *no sources at all*,
  in a corpus that mandates provenance marks at load-bearing joints.
- Statistical collapse at solo volume (finding 3), extended with arithmetic: per-class
  Service Record n of ~3–8 over months; seat swaps (Fable→Opus→Sonnet) *designed in*,
  destroying cross-campaign comparability; the 30-campaign decommission criterion
  unfalsifiable in practice; Boot Camp graduation thresholds firing on operator fatigue,
  not learned taste (the field precedent's "few hundred" verdicts quietly became "a few
  dozen" in CONCEPT.md); the Archive utility-EMA confidently ranking unattributable
  noise.
- Criticism metabolized (finding 9) — confirmed with the queue evidence: the session
  ledger ranks folding the reviews into doctrine v2.x above running anything.

**Additive (his findings the Claude reads underweighted):**
- **No deletion has ever stayed deleted.** The corpus celebrates units "recalled from
  retirement"; retirement reverses, growth never does. The pruning machinery (caps,
  demotion, decommission) has never once fired in the deleting direction.
- **"One owner per fact" is violated by the doc set itself**: the Fable window date
  lives in five-plus files, the five moves are explained in six documents, the pitch
  page was rebuilt twice in one day to stay in sync. Each file is "thin"; the set is a
  hand-synchronized fat system — the maintenance liability "doctrine, not software" was
  supposed to dissolve.
- **The git contradiction**: "the compounding lives in the files" and the files are
  untracked, uncommitted, on one laptop — one `git clean -fd` from nonexistence.
- **The human is the bottleneck, not a gate**: Red confirms, Rung-3 judging, probe
  iterations, Boot Camp reviews, First Contact, FRAGO acks, dispositions, AARs,
  check-backs — the apparatus is sized as if Commander attention were abundant, for the
  one resource that is scarcest by construction.
- **The vocabulary tax in aggregate**: ~20 costume terms must be held to operate what is
  underneath a four-verb loop (sharpen, build, attack, record).
- **The Claude-family blind spot, named**: a Claude reviewer grades the prose as the
  product, credits self-awareness as rigor, and praises the deletions while missing that
  the same day re-added more than it cut. (This file should be read with that caveat
  applied to itself.)

**Divergent:**
- He ranks the Base spec among the top-severity items ("a vanity surface pre-emptively
  defended against the vanity charge"). I hold my milder read: its build-order
  discipline is real; its existence on day one is the problem, not its content.
- His keep-list closely matches mine (Orders-as-contract, FRAGO, RoE-by-danger,
  cross-model Siege with disposition ledger, AAR-from-the-attacker's-kill-log, Proving
  Ground's honest framing) — with one addition worth quoting: the seventh thing worth
  keeping is "the already-shipped substrate the corpus admits it duplicates — forge,
  grill-me, the codex review path, ArtLab. The value was already on disk; the doctrine
  is a costume over it."
