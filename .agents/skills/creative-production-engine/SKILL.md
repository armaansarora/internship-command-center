...
9. Execute generation, ingest, QA, review board, promotion, and app integration through scripts.
   ...
   - `art:produce -- --answer <run-id> "<plain English answer>"` records Armaan's answer into durable run files and advances legal gates from files, not chat memory. When the answer approves a concept board direction, the command continues automatically through production until a true blocker or the final upload-ready board. When the answer is the exact phrase `approved for app` at the final board, the command must run the reusable promotion adapter immediately: derive approved public renditions, copy them into `public/art`, update production manifests, write the promotion receipt, and move the run into app integration without another human stop.
...
10. Promote only after the exact phrase `approved for app`. Once that exact phrase is received for a clean final board, do not stop at `approved-for-app`; that phase is an internal checkpoint that must continue through promotion and app integration automatically.
