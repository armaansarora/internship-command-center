 ...
 * once schema is pushed (Task 0.9 blocker: migration-full.sql).
 */
...
/**
 * PenthouseClient — The hero dashboard.
 *
 * Layout:
 * - Top: Greeting + time-of-day awareness
 * - Middle: 4 stat cards (glass + gold)
...
export function PenthouseClient({
  ...
  return (
    <div className="flex min-h-dvh flex-col p-6 md:p-10 gap-8 max-w-5xl mx-auto">
      {/* ── Header ── */}
      <header className="space-y-2">
        <h1 className="text-display text-xl">
          {greeting}, <span className="text-[var(--gold)]">{displayName}</span>
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="glass-card gold-border-top p-5 flex flex-col gap-3"
          >
            ...
            <div className="flex items-end gap-2">
              ...
              {stat.delta && (
                ...
                >
                  {stat.deltaDirection === "up" ? "↑" : stat.deltaDirection === "down" ? "↓" : "→"}
                  {stat.delta}
                </span>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          <button
            key={action.label}
            className="glass glass-hover p-4 text-left group cursor-default opacity-50"
            disabled
