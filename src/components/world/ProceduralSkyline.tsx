...
/* ─────────────────────────────────────────────
   PROCEDURAL NYC SKYLINE v5 — Performance-optimized

   Changes from v4 (BUG-006 / BUG-009):
   - REMOVED all mouse tracking and mouse-reactive parallax
   - REPLACED with autonomous Apple TV-style slow drift (Ken Burns)
   - Reduced building/particle counts for GPU budget
   - Removed: constellation lines, light sweep, mouse spotlight, scan line
   - Kept: stars, shooting stars, clouds, lightning, buildings, water, particles
   - Drift is purely autonomous: 60–120s sinusoidal panning, never jarring
   - prefers-reduced-motion disables all animation (static frame)
   ───────────────────────────────────────────── */
...
/* Parallax factors per depth layer — now driven by autonomous drift, not mouse */
...
function generateScene(w: number, h: number) {
  ...
  // ── BUILDINGS (reduced: 50/40/28 from 65/50/35) ──
  const buildings: Building[] = [];
  const configs = [
    { depth: 0, count: 50, minH: 0.08, maxH: 0.30, minW: 10, maxW: 32, colors: [[18, 22, 42], [22, 26, 48], [15, 18, 36], [20, 24, 44]] as [number, number, number][] },
    { depth: 1, count: 40, minH: 0.14, maxH: 0.48, minW: 18, maxW: 50, colors: [[22, 26, 50], [28, 32, 58], [20, 24, 46], [25, 28, 52]] as [number, number, number][] },
    { depth: 2, count: 28, minH: 0.20, maxH: 0.60, minW: 25, maxW: 75, colors: [[30, 34, 60], [35, 40, 68], [26, 30, 55], [38, 42, 72]] as [number, number, number][] },
  ];
...
export function ProceduralSkyline({ floorId, className = "" }: Props): JSX.Element {
  ...
  // NO mouse tracking — BUG-009: replaced with autonomous drift
  ...
  const render = useCallback((t: number) => {
    ...
    // ── AUTONOMOUS DRIFT (Apple TV screensaver style) ──
    // Slow sinusoidal panning — takes ~60-90s per cycle, never jarring
    // Two independent sine waves with different periods create organic path
    ...
    // ── BUILDINGS BY DEPTH — drift replaces mouse parallax ──
    for (let d = 0; d <= 2; d++) {
      const layer = scene.buildings.filter((b) => b.depth === d);
      const px = driftX * w * PARALLAX[d] * 12; // gentle autonomous offset
      const py = driftY * h * PARALLAX[d] * 6;
    ...
    // Shimmer lines on water (reduced from 30 → 18)
    for (let i = 0; i < 18; i++) {
