...
beforeEach(() => {
  workspaceRoot = mkdtempSync(join(tmpdir(), "artlab-srv-brain-"));
  canonRoot = mkdtempSync(join(tmpdir(), "artlab-srv-brain-canon-"));
  packsRoot = mkdtempSync(join(tmpdir(), "artlab-srv-brain-packs-"));
...
describe("server brain wiring", () => {
  it("when env has ANTHROPIC_API_KEY, generate calls produce a brainHint", async () => {
    const built = createArtLabMcpServer({
      ...
      providerProbes: {},
      version: "test",
      env: { ANTHROPIC_API_KEY: "sk-test", ARTLAB_BRAIN_MODEL: "claude-test" },
