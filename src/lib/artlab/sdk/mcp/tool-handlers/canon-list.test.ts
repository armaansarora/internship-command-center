...
import { handleArtLabCanonList } from "./canon-list";

let canonRoot: string;

beforeEach(() => {
  canonRoot = mkdtempSync(join(tmpdir(), "artlab-canon-"));
...
describe("handleArtLabCanonList", () => {
  it("returns every entry when no filter is passed", async () => {
    const result = await handleArtLabCanonList({}, { canonRoot });
  ...
  it("filters by kind when a kind is supplied", async () => {
    const result = await handleArtLabCanonList({ kind: "character" }, { canonRoot });
  ...
  it("returns an empty list (not an error) for an unknown filter result", async () => {
    const result = await handleArtLabCanonList({ kind: "palette" }, { canonRoot });
