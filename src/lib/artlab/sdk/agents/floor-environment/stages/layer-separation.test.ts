...
import { buildArtLabFloorComposite } from "./layer-separation";
...
describe("buildArtLabFloorComposite (single-composite, honest)", () => {
  it("returns exactly one composite layer flagged kind=single-composite", async () => {
    const composite = await makePng(64, 36);
    const result = await buildArtLabFloorComposite(composite);
  ...
  it("rejects a composite with no dimensions", async () => {
    await expect(
      buildArtLabFloorComposite(Buffer.from([])),
    ).rejects.toThrow();
  });
});
