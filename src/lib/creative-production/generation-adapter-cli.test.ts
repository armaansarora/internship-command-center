...
describe("art:generate CLI", () => {
  ...
  it("prepares initial design API plans as five total prompt-only concepts", () => {
    ...
    expect(prompts).toContain("Shared Tower Character Style Envelope");
    expect(prompts).toContain("premium stylized high-detail app/game character art");
    expect(prompts).toContain("Otis-compatible Tower character style");
    expect(prompts).toContain("controlled Tower lobby lighting");
    expect(prompts).toContain("full-body 9:16 character concept");
    expect(prompts).toContain("Design variation matrix");
    expect(prompts).toContain("Lane design card");
    expect(prompts).toContain("same premium finish");
    expect(prompts).toContain("Do not vary rendering style, quality, camera/framing, lighting, source model, or Tower-world fit");
    expect(plan.slots.every((slot) => slot.prompt.includes("Shared initial-concept lane quality floor"))).toBe(true);
    expect(plan.slots.every((slot) => slot.prompt.includes("Unique character design mandate"))).toBe(true);
